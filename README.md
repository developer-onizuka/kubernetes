...
| --- | --- | --- | --- | --- |
| Internal IP | Each Master/Worker | 192.168.122.183, etc | between Host Machine and Master/workers | IP address of the node accessible only from within the cluster. An eth0 device of the node may be chosen as Internal IP. But you also can let kubelet manage the Internal IP as like "KUBELET_EXTRA_ARGS=--node-ip=192.168.33.100" in /etc/default/kubelet.|
